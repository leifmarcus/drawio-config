...
export const CustomFonts: React.FC<CustomFontsProps> = () => {
    ...
    const description: ReactNode = (
        <div>
            Custom Fonts will be shown inside the font selection dropdown in diagrams.net. If you want to use Google
            fonts, you need to make sure that the font will be loaded when working with diagrams.net. Your Font might be
            a custom font, that is defined in a CSS file on any server. In order to add a custom font, you need to
            specify the url to a css file that defines the font. (draw.io 9.2.4 and later)
        </div>
    );
